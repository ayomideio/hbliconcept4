...
import pandas as pd
from pendingcalloverforcontrol.models import IC4_ACCEPTED_CALLOVER,IC4_Callover
from iconcept4.models import IC4_CALLOVER_EXCEPTION,Profile,IC4_Observation_Model
from .queries import *
...
def index(request):
    ...
    if('userparam' in request.session):
        ...
        if('transLimit' in request.session):
            ...
            useractivity(request.session.session_key,"Pending Callover for Control Special",socket.gethostbyname(socket.gethostname()),socket.gethostname(),request.session['username'],
                                    today_date,"""{0} visited the first summary page""".format(request.session['username']),"Pending Callover for Control Special",request.session.session_key,"U-review",
            ...
            result = cursor.execute('''
                        SELECT IC4_BRANCH_CODE, GRP_BY_DATE, SUM(CALL_NO_OF_VOUCHERS) as NO_OF_ENTRIES, SUM(CALL_CREDIT_FREQ) as NO_OF_CREDIT, SUM(CALL_DEBIT_FREQ) as NO_OF_DEBIT, TO_CHAR(SUM(CALL_CREDIT_TOTAL), 'FM99,999,999,999,999.00') as CREDIT_TOTAL_CALL, TO_CHAR(SUM(CALL_DEBIT_TOTAL), 'FM99,999,999,999,999.00') as DEBIT_TOTAL_CALL 
                                                ...
                                                FROM U_IC4INDEP.CALLOVER_WAITING_L3 A
                                                WHERE A.BRANCH_CODE IN ({0})
                                                AND A.CALLOVER_OFFICER IS NULL
                                                AND A.TXN_MNEMONIC='NORMAL'
                                                AND A.CALLOVER_OFFICER IS NULL
                                                AND EXISTS ( SELECT DISTINCT B.TRANS_ID FROM U_IC4INDEP.CALLOVER_WAITING_L3 B
...
def Bank_Trans(request, branch_id):
    ...
    if('userparam' in request.session):
        ...
        cursor = connection.cursor()
        useractivity(request.session.session_key,"Pending Callover for Control Special",socket.gethostbyname(socket.gethostname()),socket.gethostname(),
            ...
            branch_id),"Pending Callover for Control Special",request.session.session_key,"U-review",
            "Callover",today,today_date)
        result = cursor.execute("""
        ...
        return render(request, 'pendingcalloverforcontrolspecial/branch_trans.html', {'results': yList,'username':request.session['userparam'],'grouplist':request.session['grouplist'],'grouplistbsm':request.session['grouplistbsm'],'grouplistbc':request.session['grouplistbc'],'grouplisttrs':request.session['grouplisttrs'],'grouplisttrc':request.session['grouplisttrc'],'grouplistcon':request.session['grouplistcon'],'grouplistfot':request.session['grouplistfot'],'grouplistfoc':request.session['grouplistfoc'],'grouplist_ce':request.session['grouplist_ce'],'grouplist_ca':request.session['grouplist_ca'],'grouplist_db':request.session['grouplist_db'],'grouplist_ei':request.session['grouplist_ei']})
    else:
        return HttpResponseRedirect(reverse('login'))
...
def acc_trans(request, trans_id):
    ...
    if('userparam' in request.session):
        ...
        useractivity(request.session.session_key,"Pending Callover for Control Special",socket.gethostbyname(socket.gethostname()),socket.gethostname(),
            ...
            trans_id),"Pending Callover for Control Special",request.session.session_key,"U-review",
            "Callover",today,today_date)
        ...
        if (yList):
            ...
            # print(tst)
            # print("yList:",yList)
            transections = IC4_Callover.objects.filter(IC4_Inputter=trans_id)
            ...
            if(list2):
                ...
                useractivity(request.session.session_key,"Pending Callover for Control Special",socket.gethostbyname(socket.gethostname()),socket.gethostname(),
                                ...
                                list2[1],request.session['profile']),"Pending Callover for Control Special",request.session.session_key,"U-review",
                                "Callover",today,today_date)
                cursor13.execute("""
                                    SELECT 
                                        ID,
                ...
                transections = IC4_Callover.objects.filter(IC4_Inputter=list2[3])
                request.session['tranid'] =b_c
                for t in transections:
                    vouchers.append(t.Ref_Num)
                    # print("Ref_Num:", t.Ref_Num)
...
COD_BANK, TXN_MNEMONIC, TXN_BRANCH, AMOUNT, TXN_CODE,TIME_KEY, REF_NUM FROM callover_WAITING_l3 WHERE  
...
def OCR(request):
    ...
    initialcursor.execute(''' 
                                SELECT IC4_BRANCH_CODE, GRP_BY_REF,GRP_BY_DATE, CALLOVER_OFFICER, GRP_BY_USER, TXN_CODE,
                                    ...
                                    AMT_FIELD_1, AMT_FIELD_2, DATE_FIELD_1, DATE_FIELD_2 , CALLOVER_REMARK, EXCEPTION_ID, SEVERITY_ID, REVIEW_DATE, CALLOVER_OFFICER, CALLOVER_DATE, 
                                    CALLOVER_TIME, IC4_SPECIAL, CHECKER_DATE_TIME, CALLOVER_ID, POST_BRN, BATCH_NO, V_NUM, DOC_NO, DR_TXN_NGN, RATE, TXN_MNEM AS MNEM, AC_GL_BRN_NAME, 
                                    COD_BANK, TXN_MNEMONIC, TXN_BRANCH, AMOUNT, TXN_CODE,TIME_KEY, REF_NUM FROM CALLOVER_WAITING_L3 A 
    ...
    if(request.POST.get('singletran')):
        print('yes of single')
        transections = IC4_Callover.objects.filter(Trans_ID=request.POST.get('singletran'))
        cursor=connection.cursor()
        for transection in transections:
            cursor.execute(''' 
                UPDATE CALLOVER_WAITING_L3
            SET CALLOVER_DATE = %s, 
            CALLOVER_TIME = %s,
            CALLOVER_ID = %s, 
            CALLOVER_OFFICER = %s
        ...
        if(status=="True"):
            ...
            transections = IC4_Callover.objects.filter(IC4_Inputter=id, Entry_Date=request.session['transactionDate'])
            ...
            for transection in transections:
                ...
                if (abs(float(transection.LCY_Amount)) >= abs (float(request.session['transLimit']))):
                    ...
                    "Pending Callover for Control Special",request.session.session_key,"U-review","Callover",today,today_date)
