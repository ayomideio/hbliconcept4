...
def indexoflogin(request):
    if(request.POST.get('username')):
        ...
        # query_usergroup = __get_user_group(username)
       

        


        # Usergroup = query_usergroup['usergroup']
        # print('Usergroup:',Usergroup)


        # LdapConnectionUrl = 'ldap.forumsys.com'
        # LdapAuthenticationDomain = 'forumsys.com'
        # LdapUserDn = 'dc=example,dc=com'
        ...
        if LdapAuthentication.upper()=='YES':
            print('AD enabled')
            user = authenticate(request, username=username, password=password,LdapConnectionUrl=LdapConnectionUrl,LdapAuthenticationDomain=LdapAuthenticationDomain,LdapUserDn=LdapUserDn)
            print('Returned User',user)
            if user:
                 return HttpResponseRedirect(reverse('index'))
            else:
                return render(request, 'allauth/account/login.html', {"error": "Authentication Fail !!"})
        else:
...
def index(request):
    ...
    result = cursor.execute('''SELECT IC4_BRANCH_CODE, GRP_BY_DATE, SUM(CALL_NO_OF_VOUCHERS) as NO_OF_ENTRIES, SUM(CALL_CREDIT_FREQ) as NO_OF_CREDIT, SUM(CALL_DEBIT_FREQ) as NO_OF_DEBIT, TO_CHAR(SUM(CALL_CREDIT_TOTAL), 'FM99,999,999,999,999.00') as CREDIT_TOTAL_CALL, TO_CHAR(SUM(CALL_DEBIT_TOTAL), 'FM99,999,999,999,999.00') as DEBIT_TOTAL_CALL 
        ...
        ORDER BY GRP_BY_DATE, IC4_BRANCH_CODE
         '''.format(b_c,request.session['ldapId']), None)
    ...
    query3 = '''SELECT GROUP_NAME FROM USERROLE WHERE USER_ID ='{0}'  '''.format(username)
    cursor3=connection.cursor()
    cursor3.execute(query3)
    yList3 = dictfetchall(cursor3)
    # grouplist=''
    for index in range(len(yList3)):
                for key in yList3[index]:
                    ...
                    query31 = '''SELECT * FROM ROLE WHERE MENU_HEAD='Teller' AND GROUP_ID_ID ='{0}'  '''.format(usergroup)
                    ...
                    query32 = '''SELECT * FROM ROLE WHERE MENU_HEAD='BSM/HTS' AND GROUP_ID_ID ='{0}'  '''.format(usergroup)
                    ...
                    query34 = '''SELECT * FROM ROLE WHERE MENU_HEAD='Treasury Operation' AND GROUP_ID_ID ='{0}'  '''.format(usergroup)
...
def Bank_Trans(request, branch_id):
    ...
    AND A.USER_ID= '{1}'
     ...
     """.format(branch_id,request.session['ldapId']), None)
    ...
    return render(request, 'callover/branch_trans.html', {'results': yList,'username':request.session['userparam'],'grouplist':request.session['grouplist'],'grouplistbsm':request.session['grouplistbsm'],'grouplistbc':request.session['grouplistbc'],'grouplisttrs':request.session['grouplisttrs'],'grouplistcon':request.session['grouplistcon']})
...
ORDER BY DESCRIPTION
    ...
    SELECT 
        ...
        AND TXN_MNEMONIC = 'NORMAL'
        AND CALLOVER_OFFICER IS NULL
        AND  ACCOUNT_NAME not like 'TILL-INTER%' """.format(trans_id,request.session['selectedbranch'] ,tst,request.session['ldapId']),None)
    ...
    AND A.USER_ID= '{1}'
     ...
     """.format(request.session['selectedbranch'],request.session['ldapId']), None)
        ...
        cursor13.execute("""
        SELECT ID, Trans_ID, TRANS_REF_ID, TRANS_REF_ID AS REF_ID, TRANS_SUB_ID AS AC_ENTRY_SR_NO, BRANCH_CODE, BRANCH_NAME, 
        ACCOUNT_ID AS ACCOUNT_NUMBER, ACCOUNT_NAME AS CUSTOMER_ACCOUNT_NAME , LCY_CODE,LCY_AMOUNT, FCY_AMOUNT, FCY_CODE, TRANS_CODE  
        AS TXN_C, TRANS_MODE as TRN_REF_NO, CHEQUE_NO, NARRATIVE AS TRANSACTION_NARRATIVE, TO_CHAR(ENTRY_DATE,'YYYY-MM-DD') AS VALUE_DATE, TO_CHAR(TRAN_DATE,'YYYY-MM-DD')TRAN_DATE, BOOKING_DATE, 
        POSTING_DATE, ENTRY_DATE, ENTRY_TIME, IC4_ACCOUNT_OFFICER, IC4_INPUTTER , IC4_AUTHORISER , IC4_VERIFIER, CHECKER_ID, MAKER_ID, TEXT_FIELD_1
        ,TEXT_FIELD_2, AMT_FIELD_1, AMT_FIELD_2, DATE_FIELD_1, DATE_FIELD_2 , CALLOVER_REMARK, EXCEPTION_ID, SEVERITY_ID, REVIEW_DATE,
        CALLOVER_OFFICER, CALLOVER_DATE, CALLOVER_TIME, IC4_SPECIAL, CHECKER_DATE_TIME, CALLOVER_ID, POST_BRN, BATCH_NO, V_NUM, DOC_NO,
        DR_TXN_NGN, RATE, TXN_MNEM AS MNEM, AC_GL_BRN_NAME, COD_BANK, TXN_MNEMONIC, TXN_BRANCH, AMOUNT, TIME_KEY,
        REF_NUM FROM callover_transaction WHERE ENTRY_DATE='{2}'  AND  Branch_code='{1}' AND 
        TXN_MNEMONIC = 'NORMAL' AND CALLOVER_OFFICER IS NULL AND  ACCOUNT_NAME not like 'TILL-INTER%' AND 
        Trans_ID = '{0}' """.format(list2[1],request.session['selectedbranch'],list2[2]),None)
...
def Exceptions(request):
    ...
    exception_transection = IC4_CALLOVER_EXCEPTION(CallOver_ID=calloverID, Owner_Detail=bsmheadteller,Callover_Level=Calloverlevel, Branch_Code= branchCode, Maturity_Rating=transID, Inputter_Email=inputterEmail, Callover_Officer=officer, Severity_Level=Severity, Implication=implication, Action=action, Exception_Detail=exceptionDetails, Observation=observation, Issue_Priority=enteryDate, Callover_Date=calloverDate, Review=reviewDate,Supervisor=bsmheadteller,Tree_ID=5188)
...
def OCR(request):
    ...
    # sign_result, stamp_result, tesser_amount_word, decoded_line_am, decoded_line_denoiser, tesser_amount_fig, pdf_date = numbers_to_strings(argument, "ticket.jpg")
     ...
     # response.profileId+''+query_result.date2 +''+response.current_tim2+''+query_result.trans_id

    my_id= str(profileId)+str(query_result['date2'])+str(current_tim2)+str(query_result['trans_id'])
    print('New CalloverId',my_id)
