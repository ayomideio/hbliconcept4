...
from .models import *
from iconcept4.models import Profile,IC4_Observation_Model, IC4_CALLOVER_EXCEPTION
...
def index(request):
    ...
    if('userparam' in request.session):
        ...
        if('transLimit' in request.session):
            ...
            useractivity(request.session.session_key,"Pending Callover for Control",socket.gethostbyname(socket.gethostname()),socket.gethostname(),request.session['username'],
                                    today_date,"""{0} visited the first summary page""".format(request.session['username']),"Pending Callover for Control",request.session.session_key,"U-review",
                                        "Callover",today,today_date)
            request.session['profile']="Select Profile"
            if (request.POST.get('profile')):
                useractivity(request.session.session_key,"Pending Callover for Control",socket.gethostbyname(socket.gethostname()),socket.gethostname(),
            request.session['username'],
            today_date,"""{0} visited the first summary page and grouped by profile {1}""".format(request.session['username'],
            request.POST.get('profile')),"Pending Callover for Control",request.session.session_key,"U-review",
            "Callover",today,today_date)
            ...
            profileresult=cursor_profile.execute('''
                        ...
                        FROM U_IC4INDEP.CALLOVER_WAITING_L3 T 
...
def Bank_Trans(request, branch_id):
    ...
    if('userparam' in request.session):
        ...
        useractivity(request.session.session_key,"Pending Callover for Control",socket.gethostbyname(socket.gethostname()),socket.gethostname(),
            ...
            branch_id),"Pending Callover for Control",request.session.session_key,"U-review",
            "Callover",today,today_date)
        ...
        if(request.POST.get('profile')):
            ...
            branch_id,request.POST.get('profile')),"Pending Callover for Control",request.session.session_key,"U-review",
            "Callover",today,today_date)
            request.session['profile']=request.POST.get('profile')
            ...
            if(request.session['profile']  != "Select Profile"):
                useractivity(request.session.session_key,"Pending Callover for Control",socket.gethostbyname(socket.gethostname()),socket.gethostname(),
                    ...
                    branch_id,request.session['profile']),"Pending Callover for Control",request.session.session_key,"U-review",
                    "Callover",today,today_date)
...
def acc_trans(request, trans_id):
    ...
    if('userparam' in request.session):
        ...
        if(checkSkip=="skip"):
            ...
            trans_id),"Pending Callover for Control",request.session.session_key,"U-review",
            "Callover",today,today_date)
            if(request.session['profile']=="Select Profile"):
                useractivity(request.session.session_key,"Pending Callover for Control",socket.gethostbyname(socket.gethostname()),socket.gethostname(),
                    request.session['username'],
                ...
                useractivity(request.session.session_key,"Pending Callover for Control",socket.gethostbyname(socket.gethostname()),socket.gethostname(),
                        ...
                        trans_id,request.session['profile']),"Pending Callover for Control",request.session.session_key,"U-review",
                        "Callover",today,today_date)
                results = cursor.execute("""
                ...
                """,{'branchcode':request.session['selectedbranch'],'dater':request.session['transactionDate']})
                    ...
                    if (list2):
                        ...
                        useractivity(request.session.session_key,"Pending Callover for Control",socket.gethostbyname(socket.gethostname()),socket.gethostname(),
                                ...
                                list2[1],request.session['profile']),"Pending Callover for Control",request.session.session_key,"U-review",
                                "Callover",today,today_date)
                        cursor13.execute("""
                                        SELECT 
                                    ID,
...
def Observation(request):
    ...
    useractivity(request.session.session_key,"Pending Callover for Control",socket.gethostbyname(socket.gethostname()),socket.gethostname(),
                    ...
                    Selected),
                    "Pending Callover for Control",request.session.session_key,"U-review","Callover",today,today_date)
